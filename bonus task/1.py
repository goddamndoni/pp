...
inline_keyboard = pyrogram.types.InlineKeyboardMarkup([
    ...
    [
        pyrogram.types.InlineKeyboardButton(
            text='by:Danabek Raimkhanov',
            url='https://t.me/goddamndoni'
        ),
        pyrogram.types.InlineKeyboardButton(
            text='github',
            url="https://github.com/goddamndoni"
        )
    ]
])
...
bot = pyrogram.Client("DonisRegistrationBot", api_id=TELEGRAM_API_ID, api_hash=TELEGRAM_API_HASH, parse_mode=pyrogram.enums.ParseMode.HTML)
...
def command_start(_, message):
    ...
    cursor.execute('SELECT phonenumber FROM people WHERE id = %s', (str(message.from_user.id),))
    if cursor.fetchone() == None:
        message.reply(
            '''Здравствуйте, это бот для записи телефонных номеров, кажется вы не зарегистрированы.''',
            reply_markup=inline_keyboard)
    else:
        cursor.execute('SELECT name, surname FROM people WHERE id = %s', (str(message.from_user.id),))
        res = cursor.fetchall()
...
def help(bot, message):
    cursor.execute('SELECT surname, name FROM people WHERE id = %s', (str(message.from_user.id),))
    res = cursor.fetchall()
    message.reply(
        f'''Hey {res[0][0]} {res[0][1]}, how you doing ?
        
This bot is created by @goddamndoni.
...
def getme(bot, message):
    cursor.execute("SELECT * FROM people WHERE id = %s", (str(message.from_user.id),))
    res = cursor.fetchone()
    if res == []:
...
def setsurname(bot, message):
    ...
    if len(message.command) == 1 or len(message.command) > 2:
        ...
        cursor.execute('SELECT surname FROM people WHERE id = %s', (str(message.from_user.id),))
        res = cursor.fetchall()
        newSurname = message.command[1]
        message.reply(f'''You are going to change your surname
    from : {res[0][0]} 
...
def setname(bot, message):
    ...
    if len(message.command) == 1 or len(message.command) > 2:
        ...
        cursor.execute('SELECT name FROM people WHERE id = %s', (str(message.from_user.id),))
        res = cursor.fetchall()
        newName = message.command[1]
        message.reply(f'''You are going to change your name 
    from : {res[0][0]} 
...
def Callback(Client, CallbackQuery):
    ...
    if CallbackQuery.data == 'registerCallback':
        ...
        cursor.execute('UPDATE people SET surname = %s WHERE id = %s', (newSurname, str(CallbackQuery.from_user.id)))
        cursor.execute('SELECT chatid FROM people WHERE id = %s', (str(CallbackQuery.from_user.id),))
        res = cursor.fetchall()[0][0]
        bot.edit_message_text(res, messageId,'''<i>You are going to change your surname...</i>
...
Try /getme again''')
        ...
        cursor.execute('SELECT chatid FROM people WHERE id = %s', (str(CallbackQuery.from_user.id),))
        res = cursor.fetchall()[0][0]
        bot.edit_message_text(res, messageId, '''<i>You are going to cahange your surname...</i>

<b>Canceled</b>''')
    elif CallbackQuery.data == 'changeName':
        cursor.execute('UPDATE people SET name = %s WHERE id = %s', (newName, str(CallbackQuery.from_user.id)))
        cursor.execute('SELECT chatid FROM people WHERE id = %s', (str(CallbackQuery.from_user.id),))
        res = cursor.fetchall()[0][0]
        bot.edit_message_text(res, messageId,'''<i>You are going to change your name...</i>
...
Try /getme again''')
        ...
        cursor.execute('SELECT chatid FROM people WHERE id = %s', (str(CallbackQuery.from_user.id),))
        res = cursor.fetchall()[0][0]
        bot.edit_message_text(res, messageId, '''<i>You are going to cahange your name...</i>
...
def contact(bot, message):
    ...
    cursor.execute('SELECT phonenumber FROM people WHERE username = %s', (username,))
    if cursor.fetchall() == []:
        phoneNumber = message.contact.phone_number
        SQLcontactRegister = f'INSERT INTO people VALUES(%s, %s, %s, %s, %s, %s);'
